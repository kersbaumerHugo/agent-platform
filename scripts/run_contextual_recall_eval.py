...
class ContextualRecallEvalDataset(BaseModel):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the deterministic M9 contextual recall evaluation suite."
    )
    parser.add_argument(
        "--dataset",
        type=Path,
        default=Path("evals/contextual_recall/m9_v0.json"),
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def load_dataset(
    path: Path,
) -> ContextualRecallEvalDataset:
    return ContextualRecallEvalDataset.model_validate_json(path.read_text(encoding="utf-8"))


async def execute_suite(
    dataset: ContextualRecallEvalDataset,
    database_path: Path,
...
async def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-platform-m9-context-eval-") as first_temp_dir:
        first_results = await execute_suite(
            dataset,
            Path(first_temp_dir) / "memory.sqlite3",
        )

    with tempfile.TemporaryDirectory(prefix="agent-platform-m9-context-eval-") as second_temp_dir:
